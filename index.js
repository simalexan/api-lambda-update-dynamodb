...
exports.handler = async event => {
  ...
  if (!event.body) {
    return processResponse(IS_CORS, 'no body arguments provided', 400);
  }
  const editedItemId = event.pathParameters.id;
  if (!editedItemId) {
    return processResponse(IS_CORS, 'invalid id specified', 400);
  }

  const editedItem = JSON.parse(event.body);
  const editedItemProperties = Object.keys(editedItem);
  if (!editedItem || editedItemProperties.length < 1) {
    return processResponse(IS_CORS, 'no args provided', 400);
  }
